...
def test_listen(webserver, port):
    ...
    with webserver("127.0.0.1", f"{port}"), socket.create_connection(
        ("localhost", port)
    ):
        pass
...
def test_packets(webserver, port):
    ...
    with webserver("127.0.0.1", f"{port}"), socket.create_connection(
        ...
        time.sleep(
            0.5
        )  # Attempt to gracefully handle all kinds of multi-packet replies...
...
def test_httpreply(webserver, port):
    ...
    with webserver("127.0.0.1", f"{port}"), socket.create_connection(
        ...
        reply = conn.recv(1024)
        print("pytest: ", reply)
        assert re.search(rb"HTTP/1.[01] 400.*\r\n(.*\r\n)*\r\n", reply)
...
def test_httpreplies(webserver, port):
    ...
    with webserver("127.0.0.1", f"{port}"), contextlib.closing(
...
def test_static_content(webserver, port):
    ...
    with webserver("127.0.0.1", f"{port}"), contextlib.closing(
        ...
        for path in ["/static/other", "/static/anything", "/static/else"]:
            ...
            reply = conn.getresponse()
            reply.length = (
                0  # Convince `http.client` to handle no-content 404s properly
            )
            reply.read()
            assert reply.status == 404
...
def test_dynamic_content(webserver, port):
    ...
    with webserver("127.0.0.1", f"{port}"), contextlib.closing(
        ...
        assert (
            response.status == 404
        ), f"'{path}' should be missing, but GET was not answered with '404'"
        ...
        assert response.status in {
            ...
            202,
            204,
        }, f"Creation of '{path}' did not yield '201'"
        ...
        assert response.status in {
            ...
            202,
            204,
        }, f"Deletion of '{path}' did not succeed"
